...
class Intermediate {
    ...
    */
    constructor() {
        ...
        this.classStack = new stack_1.Stack();
        this.operandStack = new stack_1.Stack();
        this.operatorStack = new stack_1.Stack();
        this.dim = [];
        this.r = 1;
        this.dimStack = new stack_1.Stack();
        this.quadruples = new Array();
        this.jumpsStack = new stack_1.Stack();
        this.initialGoto = 0;
        this.scopeStack = new stack_1.Stack();
    ...
    saveParam(name, type) {
        ...
        this.varTable.forEach((variable) => {
            if (variable.varName === name &&
                ...
                this.dirFunc[this.scopeStack.peek()].parameterTable.push(variable.dir.toString());
            }
        });
    }
    /*
    ...
    */
    saveArray(variable, type, expression, isConst) {
        // If the variable is already declared in the current scope return error
        if (this.varTable.find((x) => x.varName === variable &&
            ...
            this.dirFunc[this.scopeStack.peek()].scope;
            const dir = this.dirFunc[this.scopeStack.peek()].scope * SCOPE_MEM_SIZE +
                (isConst ? 500 : 0) +
                getTypeDir(type) +
    ...
    saveObjectVariable(variable, type, expression, isConst) {
        // If the variable is already declared in the current scope return error
        if (this.varTable.find((x) => x.varName === variable &&
            (Math.floor(x.dir / SCOPE_MEM_SIZE) ===
                this.dirFunc[this.scopeStack.peek()].scope ||
    ...
    */
    saveVariable(variable, type, expression, isConst) {
        // If the variable is already declared in the current scope return error
        if (this.varTable.find((x) => x.varName === variable &&
            (Math.floor(x.dir / SCOPE_MEM_SIZE) ===
                this.dirFunc[this.scopeStack.peek()].scope ||
    ...
    */
    closeFunction() {
        if (this.scopeStack.peek() !== "global" &&
    ...
    callFunction(funcName) {
        ...
        if (!func) {
            ...
            if (func.type === "void") {
                ...
                const tempDir = this.tempStack.size() +
    ...
    */
    storeConstOperand(operand, type) {
    ...
    */
    storeVariableOperand(variable) {
        const varDir = this.varTable.find((x) => x.varName === variable);
        if (!varDir) {
        ...
        if (this.scopeStack.peek() !== "global" &&
            ...
            if (func.type === "void") {
                ...
                if (this.operandStack.peek().type !== func.type) {
                    throw new Error(`Return type (${this.operandStack.peek().type}) does not match function type (${func.type}).`);
                }
                this.quadruples.push({
                    op: "RETURN",
    ...
    */
    ...
    processBooleanExpression(operand1, operand2, operator) {
        if (operator === ">") {
    ...
    */
    getOperatorType(operator, operand1, operand2) {
    ...
    */
    processDecisionStatExpression() {
        if (this.operandStack.peek().type !== "bool") {
            throw new Error(`Type mismatch with ${this.operandStack.peek().val} : ${this.operandStack.peek().type}`);
        }
        else {
    ...
    storeData(varName, data) {
        ...
        if (variable && val) {
            ...
            if (!this.matchingType(type, val.type)) {
                throw new Error(`Type mismatch in ${varName}, expected ${type} got ${val.type}`);
            }
            this.quadruples.push({
                op: "=",
                arg1: val.val,
